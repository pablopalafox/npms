...
class ViewerInterpolateShape:
    def __init__(
        ...
        view="lateral_left",
        dataset_class=None,
        src_identity_name=None,
        tgt_identity_name=None,
        ...
        self.src_identity_name = src_identity_name
        self.tgt_identity_name = tgt_identity_name
        ...
        for identity_id, label_tpose in enumerate(self.labels_tpose):
            if label_tpose['identity_name'] == self.src_identity_name:
                self.src_identity_id = identity_id
            if label_tpose['identity_name'] == self.tgt_identity_name:
        ...
        self.out_dir = os.path.join(out_root_dir, f"{self.src_identity_name}__TO__{self.tgt_identity_name}__@{self.reconstruction_res}__fps{self.frame_rate}")
    ...
    def initialize(self):
        ...
        ################################################################################################################
        shape_code_start = self.shape_codes[[self.src_identity_id], ...] # [bs, 1, C]
        shape_code_end   = self.shape_codes[[self.tgt_identity_id], ...] # [bs, 1, C]
        ...
        for i in tqdm(range(self.num_iterpolations + 1)):

            alpha = i / self.num_iterpolations

            with torch.no_grad():
                ### Prepare INTERPOLATED shape code ###
                shape_code_interp = (1 - alpha) * shape_code_start + alpha * shape_code_end
                assert shape_code_interp.shape == (1, 1, shape_codes_dim), f"{shape_code_interp} vs {(1, 1, shape_codes_dim)}"
                ref_mesh = deepsdf_utils.create_mesh_from_code(
                    shape_decoder, shape_code_interp, shape_codes_dim=shape_codes_dim,
                    N=self.reconstruction_res, max_batch=self.max_batch
                )

            COLOR_INTERP = (1 - alpha) * self.START_COLOR + alpha * self.END_COLOR
            ref_mesh = trimesh_to_open3d(ref_mesh, COLOR_INTERP)

            # o3d.visualization.draw_geometries([ref_mesh])
            
            self.ref_list.append(ref_mesh)
    ...
    def update_ref(self, vis):
        ...
        self.ref = self.ref_list[self.time]
    ...
    def run(self):
        ...
        def render(vis):
            ...
            os.system(f"ffmpeg -r {self.frame_rate} -i {self.out_dir}/interp_%02d.jpg -c:v libx264 -vf fps=30 -pix_fmt yuv420p -y {self.out_dir}/{self.src_identity_name}__TO__{self.tgt_identity_name}__@{self.reconstruction_res}.mp4")
            exit() 
...
if __name__ == "__main__":
    ...
    out_root_dir = "/cluster_HDD/lothlann/ppalafox/qualitative_results__interpolation_SHAPE"
    ...
    # src_identity_name = "kate"
    # tgt_identity_name = "adam"

    src_identity_name = "suzie"
    tgt_identity_name = "racer"

    # src_identity_name = "shae"
    # tgt_identity_name = "regina"

    view = "frontal"

    # -----------------------------------------------------------------

    viewer = ViewerInterpolateShape(
        labels,
        labels_tpose,
        shape_codes,
        out_root_dir,
        ...
        src_identity_name=src_identity_name,
        tgt_identity_name=tgt_identity_name,
        num_iterpolations=60,
        frame_rate=15,
        res_mult=2
    )
    viewer.run()